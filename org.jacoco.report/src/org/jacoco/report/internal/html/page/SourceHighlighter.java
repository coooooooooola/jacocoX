...
final class SourceHighlighter {
    ...
    public void render(final HTMLElement parent, final ISourceNode source,
        ...
        while ((line = lineBuffer.readLine()) != null) {
            ...
            if (source instanceof SourceFileCoverageImpl) {
                changeLinesInfo = ((SourceFileCoverageImpl) source).getChangeLinesInfo();
            }
    ...
    private void renderCodeLine(HTMLElement pre, String linesrc, ILine line, int lineNr, List<ChangeLineDto> changeLinesInfo) throws IOException {
    ...
    private String getLineMark(List<ChangeLineDto> lines, int lineNr) {
        ...
        for (int j = 0; j < lines.size(); j++) {
            ...
            for (int i = changeLine.getStartLineNum().intValue() + 1; i <= changeLine
                    .getEndLineNum().intValue(); ) {
                if (lineNr != i) {
                    i++;
                    continue;
                }
                return type.equals("INSERT") ? "line-add" : type.equals("REPLACE") ? "line-edit" : "";
            }
        }
        return "";
    }
